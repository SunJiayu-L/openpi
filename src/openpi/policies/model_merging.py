...
# 文件结构：
# 1) 通用校验与工具函数
# 2) 各类参数融合策略（linear / SLERP / multimodal / task-vector / DARE）
# 3) 融合策略注册表（名称 -> 函数）
...
def validate_params_list(
    ...
    # 先构建一次抽象模型状态，再检查每个 checkpoint 的参数树结构是否一致。
...
def linear_interpolation(
    ...
    # 约定从 merging_fn_kwargs 中读取 "model_mixing_coefficients"。
...
def spherical_linear_interpolation(
    ...
    # 按本文件约定，最后一个 checkpoint 作为 base/pretrained。
...
def _filter_keys(flat_param, include_patterns=None, exclude_patterns=None):
    ...
    for key, value in flat_param.items():
        # 将扁平化 tuple key 拼成路径字符串，便于用正则匹配子模块名。
        key_str = "/".join(key)
...
def _make_task_vector(base_params: at.Params, ft_params: at.Params) -> at.Params:
    # Task vector = finetuned 相对 base 的参数增量。
...
def task_vector_interpolation(
    ...
    # 先聚合所有缩放后的 task vector，再一次性加回 base 参数。
...
def dare_interpolation(
    ...
    # "scale" 控制（重缩放后）task vector 的注入强度。
