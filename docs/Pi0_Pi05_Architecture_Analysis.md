...
════════ STEP 5: suffix_out 后处理与损失 ════════
  # suffix_out: (B, 51, 1024)  ← π0 含 state_token，共 51 个 token
  # 取后 50 个 token（即 action token 对应的输出）

  v_t = action_out_proj(suffix_out[:, -50:])  → (B, 50, 32)
        [Linear(1024→32)，投影到动作空间，得到预测速度场]

  Loss = mean((v_t - u_t)²)
       = mean((predicted_velocity - target_velocity)²)
       = MSE Loss（逐动作维求均值，返回 shape (*batch, 50)）

  注：prefix_out 在训练时完全丢弃，不参与损失计算。
```
...
════════ PREFILL（KV Cache 预填充）════════
  ...
  注：prefix 只计算一次，后续 10 步循环全部复用此 kv_cache。

════════ ODE 迭代去噪（默认 10 步）════════
  x_t = ε ~ N(0, I)   ← 从纯噪声开始，shape (B, 50, 32)
  time = 1.0           ← t=1 为纯噪声，t=0 为干净动作
  dt = -1.0 / 10 = -0.1

  while time >= -dt/2:  (容差写法，等价于 time >= 0)

    suffix_tokens = embed_suffix(obs, x_t, time)
    [用当前 x_t 和 t 重新编码 suffix，prefix 不重算]
    ...
    (None, suffix_out), _ = Transformer(
        [None, suffix_tokens],   ← prefix=None，从 kv_cache 读取
        kv_cache=kv_cache,       ← prefix 的 K/V 直接复用
        ...
    )
    # 注：此时 prefix_out=None，assert prefix_out is None

    v_t = action_out_proj(suffix_out[:, -50:])   ← Linear(1024→32)，预测速度场
...
### 5.3 训练 vs 推理对 suffix_out 的处理对比

```
┌──────────────────────┬───────────────────────┬────────────────────────────┐
│                      │         训练          │            推理            │
├──────────────────────┼───────────────────────┼────────────────────────────┤
│ prefix 计算次数      │ 1次（和 suffix 一起） │ 1次（提前算好存 KV cache） │
├──────────────────────┼───────────────────────┼────────────────────────────┤
│ suffix 计算次数      │ 1次                   │ 10次（每步用新的 x_t）     │
├──────────────────────┼───────────────────────┼────────────────────────────┤
│ action_out_proj 调用 │ 1次，得到 v_t         │ 10次，每步得到一个 v_t     │
├──────────────────────┼───────────────────────┼────────────────────────────┤
│ suffix_out 用途      │ 算 MSE loss           │ Euler 积分更新 x_t         │
├──────────────────────┼───────────────────────┼────────────────────────────┤
│ prefix_out 用途      │ 完全丢弃              │ 不计算（kv_cache 代替）     │
├──────────────────────┼───────────────────────┼────────────────────────────┤
│ 最终输出             │ 标量 loss             │ 动作序列 x_0 (B, 50, 32)   │
└──────────────────────┴───────────────────────┴────────────────────────────┘

推理效率关键：prefix（图像+语言）的 KV 只算一次并缓存，
10步循环里只重算 suffix（动作部分），大幅节省计算量。
```
