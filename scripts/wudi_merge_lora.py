#!/usr/bin/env python3
"""WUDI merging for pi0.5 — LoRA variant (OptMerge Sec 4.2).

Algorithm: LoRA版本，对应 LLaMA-Factory/model_merging.py wudi_merging2。
与 wudi_merge.py（Full Fine-tuning, Sec 4.1）的核心区别：

  Full Fine-tuning (wudi_merge.py, Sec 4.1):
    - 对 (τ_i - τ̄) 做 SVD → de-centered taskvector + average_vector
    - low_rank 来自原始 τ_i 的 SVD（当前实现）
    - Adam 优化器，lr=1e-5

  LoRA (wudi_merge_lora.py, Sec 4.2):
    - 直接对 τ_i 做 SVD，不做去中心化
    - taskvector 和 low_rank 均来自同一次 SVD(τ_i)，来源统一
    - SGD 优化器（momentum=0.9），lr=1e-4
    - 仍用均值初始化融合向量（mean of task vectors）

论文理由（Sec 4.2）：
  LoRA 任务向量本身已经是低秩的（rank = lora_r），
  梯度在对应 null space 方向无效，Adam 容易通过增大范数走捷径。
  SGD 提供隐式正则化，约束范数不膨胀。
  不做去中心化是因为 LoRA 向量绝对幅值小、分布已够紧凑，
  centering 会丢失 LoRA 本身的低秩结构。

Gemma attention and FFN parameters are WUDI-merged;
other in-scope parameters use simple-averaged task vectors;
...
Usage:
    ...
    python scripts/wudi_merge_lora.py --test

    # Merge (action expert only):
    python scripts/wudi_merge_lora.py \\
        --base   /path/to/pi05_base/params \\
        --ft     /path/to/ft1/params /path/to/ft2/params \\
        --output checkpoints/merged/wudi_lora_e1_libero_goal \\
        --scope  expert1_only \\
        --iter   300 \\
        --scaling 1.0 \\
        --device cuda
"""
# 说明：
# 这个脚本对 pi0.5 的注意力/FFN参数做 WUDI LoRA 合并。
# 核心做法是：先把每层参数拆成 2D 子块，再对每个子块做直接 SVD（不去中心化）+ SGD 优化。
...
# 匹配所有 norm 层参数和 bias 参数，与 MLLMerging exclude_param_names_regex 对齐。
# 这些参数保持 base 值不变，不参与任何平均或 WUDI 融合。
...
def _is_norm_or_bias(key: str) -> bool:
    """True if key is a normalization layer or bias parameter (keep frozen at base)."""
...
def _in_scope(key: str, scope: str) -> bool:
    ...
    # scope=lang_and_vision：language expert (expert0) + vision encoder，排除 action expert (expert1)。
...
# ---------------------------------------------------------------------------
# WUDI optimization — LoRA variant (OptMerge Sec 4.2)
# ---------------------------------------------------------------------------
...
def wudi_optimize(
    ...
    """Minimize inter-task interference for a single 2D sub-block — LoRA variant.

    Implements OptMerge Sec 4.2: no de-centering, SGD optimizer.

    Key differences from wudi_merge.py (Full Fine-tuning, Sec 4.1):
      - No de-centering: both taskvector and low_rank come from SVD(τ_i) directly.
        In Sec 4.1, taskvector comes from SVD(τ_i - τ̄) and low_rank from SVD(τ_i).
        Here both share the same single SVD(τ_i), so the optimization target is
        consistent (no mixed SVD sources).
      - SGD with momentum=0.9 replaces Adam.
        Reason (paper): LoRA gradients are effective only in the non-null subspace;
        Adam's adaptive LR tends to inflate merge vector norm by exploiting null
        directions. SGD provides implicit regularization that constrains norm growth.
      - Initialization: mean of task vectors (same as Sec 4.1).

    Args:
        label:    human-readable name for logging
        vectors:  stacked task vectors, shape (T, m, n)
        iter_num: SGD optimization steps
        device:   'cuda' or 'cpu'
    ...
    # ------------------------------------------------------------------
    # LoRA Sec 4.2：直接对每个原始任务向量 τ_i 做 SVD，不做去中心化。
    # 两个列表共享同一次 SVD，保证 taskvector 与 low_rank 来源一致。
    # ------------------------------------------------------------------
    low_rank_list: list[torch.Tensor] = []   # Σ_{1:k} V_{1:k}^T，投影子空间
    taskvector_list: list[torch.Tensor] = [] # U_{1:k} Σ_{1:k} V_{1:k}^T，截断重建
    ...
    for i in range(T):
        ...
        if use_compact:
            ...
            # s: (min_dim,), u: (m, min_dim), v: (min_dim, n)
            min_dim = s.shape[0]
            ...
            # taskvector = U_{1:k} Σ_{1:k} V_{1:k}^T，截断低秩重建（无均值项）。
            taskvector_list.append(u_k @ torch.diag(s_k) @ v_k)
            # low_rank = Σ_{1:k} V_{1:k}^T，投影基（与 taskvector 同源）。
            low_rank_i = s_k.unsqueeze(1) * v_k  # (r, n)
            ...
            # taskvector = U_{1:k} Σ_{1:k} V_{1:k}^T。
            # compact SVD 做截断重建（更高效，避免构造大 S 矩阵）。
            u2, s2, v2 = torch.linalg.svd(vector, full_matrices=False)
            u2 = u2[:, :reduced_r]
            s2 = s2[:reduced_r]
            v2 = v2[:reduced_r, :]
            taskvector_list.append(u2 @ torch.diag(s2) @ v2)

            # low_rank = Σ_{1:k} V_{1:k}^T，从同一次完整 SVD 构造。
    ...
    # 堆叠后便于批量算损失。
    low_rank = torch.stack(low_rank_list)    # (T, r, n) or (T, m, n)
    taskvector = torch.stack(taskvector_list)  # (T, m, n)

    # ------------------------------------------------------------------
    # 均值初始化（论文 Sec 4.2 改进 3，ablation Table 4 +4.43%）。
    # LoRA 和 Full Fine-tuning 均使用 mean，而非 sum。
    # ------------------------------------------------------------------
    merging = torch.nn.Parameter(vectors.mean(dim=0).clone())

    # ------------------------------------------------------------------
    # SGD 优化器（论文 Sec 4.2 改进 1）。
    # LoRA 梯度仅在非零奇异值方向有效；Adam 自适应 LR 会利用零梯度方向
    # 增大范数走捷径。SGD 提供隐式正则化，抑制范数膨胀。
    # ------------------------------------------------------------------
    opt = torch.optim.SGD([merging], lr=1e-4, momentum=0.9)

    # 每个任务向量范数平方，用于归一化损失（分母用原始 τ_i，不截断）。
    ...
    for step in range(iter_num):
        ...
        # 投影到各任务低秩子空间，衡量干扰程度。
        ip = torch.matmul(diff, low_rank.transpose(-2, -1))  # (T, m, r or m)
        # 归一化平方损失之和。
...
def _load_flat(params_path: str | pathlib.Path) -> dict[str, np.ndarray]:
    ...
    # 解析 checkpoint 路径；restore_params 需要指向包含 _METADATA 的 params 子目录。
...
def run_merge(
    ...
    # 挑出可参与 WUDI 的目标参数（attn/FFN 且在 scope 内，且非 norm/bias）。
    eligible = [k for k in base if _is_attn_ffn(k) and _in_scope(k, scope) and not _is_norm_or_bias(k)]
    # 在 scope 内但非 attn/FFN 的参数（且非 norm/bias）做简单平均。
    in_scope_keys = set(k for k in base if _in_scope(k, scope))
    # norm/bias 参数始终保持 base 值，不参与任何融合。
    frozen_norm_bias = set(k for k in base if _is_norm_or_bias(k))
    logger.info(f"WUDI-eligible: {len(eligible)} / {len(base)} params  (scope={scope})")
    logger.info(f"In-scope (avg for non-attn/FFN, excl norm/bias): {len(in_scope_keys)} params")
    ...
    for key, base_val in base.items():
        # norm/bias 参数：始终保持 base 值。
        if key in frozen_norm_bias:
            merged[key] = base_val
            continue

        # 非 scope 参数（如 vision model）保持 base。
        if key not in in_scope_keys:
            merged[key] = base_val
            continue

        # 在 scope 内但非 attn/FFN → 简单平均 task vector。
        ...
        for l in range(L):
            ...
            for b in range(num_blocks):
                ...
                result, loss_summary = wudi_optimize(
                    label,
                    __import__("torch").from_numpy(block_stack),
                    iter_num=iter_num,
                    device=device,
    ...
    if all_final_losses:
        ...
        logger.info(
            "WUDI LoRA loss summary (mean over blocks): "
...
def run_test() -> None:
    ...
    # 3) WUDI LoRA 优化器小规模运行测试。
    print("\n=== WUDI LoRA optimize smoke test (CPU, 2 models, 5 steps) ===")
    vectors = torch.randn(2, 8, 16)
    result, summary = wudi_optimize("smoke", vectors, iter_num=5, device="cpu")
    assert result.shape == (8, 16)
    assert torch.isfinite(result).all()
    print(f"  shape={tuple(result.shape)}  finite={torch.isfinite(result).all().item()}  ✓")
    print(f"  first_loss={summary['first_loss']:.4e}  final_loss={summary['final_loss']:.4e}")

    # 4) 与 wudi_merge.py（Full FT）对比：验证两者行为不同（SGD vs Adam）。
    print("\n=== Optimizer difference check: SGD (LoRA) vs Adam (Full FT) ===")
    import torch.optim
    torch.manual_seed(42)
    v = torch.randn(2, 8, 16)
    # LoRA: SGD
    result_lora, _ = wudi_optimize("lora", v.clone(), iter_num=10, device="cpu")
    print(f"  LoRA  (SGD)  result norm: {result_lora.norm():.4f}")
    print("  (Full FT uses Adam — import wudi_merge.wudi_optimize to compare)")
...
def main() -> None:
    ...
    # 定义命令行参数。
    ap = argparse.ArgumentParser(description="WUDI LoRA merging for pi0.5 (OptMerge Sec 4.2)")
