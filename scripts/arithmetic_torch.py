"""
Mix multiple OpenPI PyTorch checkpoints (model.safetensors) with weighted parameter averaging.

For JAX checkpoints (Orbax/OCDBT), use arithmetic.py instead.

Example:
    python scripts/arithmetic_torch.py \\
        --config pi05_libero \\
...
def load_torch_params(checkpoint_path: str) -> dict:
    """Load PyTorch state_dict from model.safetensors as dict[str, np.ndarray]."""
    ...
    state = safetensors.torch.load_file(str(Path(resolved) / "model.safetensors"))
...
def _to_torch_batch(data_samples, device):
    """Convert (obs_dict, actions) batch from numpy/jax to torch tensors on device."""
...
def _get_torch_norm_stats(checkpoints, config, device):
    """Load norm_stats for first PyTorch checkpoint, trying multiple locations."""
...
def optimize_weights_with_gradient_descent_torch(
    ...
    """Optimize mixing weights via gradient descent (PyTorch checkpoints)."""
    print("\n" + "=" * 60)
    ...
    model = policy._model
    params_list = [load_torch_params(p) for p in tqdm(checkpoints, desc="Loading checkpoints")]
    n = len(checkpoints)
    keys = list(params_list[0].keys())

    log_weights = torch.zeros(n, device=device, dtype=torch.float32, requires_grad=True)
    ...
    for iteration in range(num_iterations):
        ...
        for k in keys:
            mixed[k] = sum(
                weights[i].detach() * torch.from_numpy(params_list[i][k]).to(device=device)
                for i in range(n)
            )
        model.load_state_dict(mixed, strict=False)
        data_samples = data_samples_list[iteration % len(data_samples_list)]
        obs_torch, actions_torch = _to_torch_batch(data_samples, device)
        observation = _model.Observation.from_dict(obs_torch)
        ...
        # Project param gradients onto each checkpoint's params -> d(loss)/d(weight_k)
        g_k = [0.0] * n
        for name, param in model.named_parameters():
            if param.grad is not None and name in params_list[0]:
                for i in range(n):
                    p_i = torch.from_numpy(params_list[i][name]).to(device=device)
...
def optimize_weights_with_adaptive_gradient_descent_torch(
    ...
    """Optimize mixing weights with loss-adaptive gradient scaling (PyTorch)."""
    print("\n" + "=" * 60)
...
def optimize_weights_greedy_torch(checkpoints, config, data_samples_list, device="cuda"):
    ...
    def evaluate_combination(indices):
        ...
        for k in keys:
            mixed[k] = sum(
                w[i] * torch.from_numpy(params_list[i][k]).to(device=device)
                for i in range(n)
            )
        model.load_state_dict(mixed, strict=False)
        total_loss = 0.0
...
def test_mixed_checkpoint_torch(config, checkpoint_path, data_samples_list, device="cuda"):
    """Load mixed PyTorch checkpoint and compute average loss on validation batches."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Mix OpenPI PyTorch checkpoints (model.safetensors) with weighted averaging. "
                    "Use arithmetic.py for JAX checkpoints."
    )
