...
def load_norm_stats(norm_stats_path: str) -> dict:
    """Load normalization statistics from JSON (raw dict form for mixing)."""
...
def mix_norm_stats(norm_stats_list: list, weights: list = None) -> dict:
    ...
    Operates on raw dict form (as loaded by load_norm_stats), not NormStats objects.
    Each entry in norm_stats_list is a dict of dicts with keys like 'mean', 'std', etc.
    """
...
def save_norm_stats(norm_stats: dict, output_path: str) -> None:
    """Save normalization statistics (raw dict form) to JSON."""
...
def compute_optimal_weights(losses):
    """Compute optimal weights based on inverse squared loss."""
